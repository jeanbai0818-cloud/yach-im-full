...
export const yachGetPayroll = {
    ...
    parameters: Type.Object({
        ...
        ], { description: "C=当月(默认), P=上月, N=下月" })),
        calId: Type.Optional(Type.String({ description: "翻页用，上次响应的 calId（如 'PAY 2026/05'）" })),
    }),
    async execute(_id, params) {
        const require = createRequire(import.meta.url);
        const ch7 = require("../../api/ch7-workbench/index.js");
        const r = await ch7.getPayroll({ page: params.page ?? "C", calId: params.calId ?? "" });
        const lines = [
            `📅 ${r.month}  calId: ${r.calId}`,
            `💰 应发: ¥${r.gross}  →  实发: ¥${r.net}`,
            ``,
            ...r.items.map((it) => `  ${it.amount.startsWith('-') ? '🔻' : '🔹'} ${it.name}: ¥${it.amount}\n` +
                it.details.map((d) => `      ${d.desc}: ¥${d.value}`).join('\n')),
        ];
        return toolResult(lines.join('\n').trim());
    },
};
/** ⭐ 批量查历史工资条（自动翻页）*/
export const yachGetPayrollHistory = {
    name: "yach_get_payroll_history",
    label: "历史工资条",
    description: "批量获取多个月工资条（自动翻页）。months=1~12，默认3个月。返回每月应发/实发汇总。",
    parameters: Type.Object({
        months: Type.Optional(Type.Integer({ minimum: 1, maximum: 12, description: "获取月数，默认3" })),
    }),
    async execute(_id, params) {
        const require = createRequire(import.meta.url);
        const ch7 = require("../../api/ch7-workbench/index.js");
        const list = await ch7.getPayrollHistory(params.months ?? 3);
        if (!list.length)
            return toolResult("未获取到工资条数据");
        const lines = list.map(r => `📅 ${r.month}  应发: ¥${r.gross}  实发: ¥${r.net}`);
        return toolResult(`共 ${list.length} 个月：\n\n${lines.join('\n')}`);
    },
};
//# sourceMappingURL=ch7-weekly.js.map